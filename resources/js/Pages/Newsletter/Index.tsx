...
const EmailCollection = ({ emailAddresses, separator, title, children }: EmailCollectionProps) => {
  ...
  return (
    <Paper elevation={1} sx={{ p: 3 }}>
      ...
      >
        <code id="newsletter-recipients">{emailAddresses.join(separator)}</code>
      </Paper>

      <Button variant="contained" color="yellow" onClick={copyToClipboard}>
        {copyLabel}
      </Button>
    </Paper>
  );
      ...
      <Stack spacing={4} p={4}>
        <Paper elevation={1} sx={{ p: 3, mb: 2 }}>
          ...
          <RadioGroup
            row
            value={separator}
            onChange={(e) => setSeparator(e.target.value as Seperator)}
          >
            <FormControlLabel value="," control={<Radio />} label="Comma (,)" />
            <FormControlLabel value={'\n'} control={<Radio />} label="New line" />
        ...
        >
          <Typography>These e-mail addresses represent members who:</Typography>

          <List>
            <ListItem>Have not opted out from newsletter emails</ListItem>
            <ListItem>
        ...
        >
          <Typography>
            These e-mail addresses represent currently active members, and should be emailed for
            urgent matters relating to membership of the space.
          </Typography>
        </EmailCollection>
      </Stack>
    </>
